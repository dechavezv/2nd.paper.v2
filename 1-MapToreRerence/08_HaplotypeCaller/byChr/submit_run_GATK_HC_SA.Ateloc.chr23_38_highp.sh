...
for i in {23..38}; 
	...
	do (
	echo "#! /bin/bash"
	echo "#$ -wd /u/scratch/d/dechavez/QB3ateloc"
	echo "#$ -l h_rt=24:00:00,h_data=16G,h_vmem=30G,arch=intel*"
	echo "#$ -t 01-4:1"
	echo "#$ -N HC_BD_${i}"
	echo "#$ -o /u/scratch/d/dechavez/QB3ateloc/log/reports"
	echo "#$ -e /u/scratch/d/dechavez/QB3ateloc/log/reports"
	echo "#$ -m abe"
	echo "#$ -M dechavezv"
	echo
	...
	echo
	echo "cd /u/scratch/d/dechavez/QB3ateloc"
	echo
	echo "export BAM=\$(ls \$(printf "%02d" "\$SGE_TASK_ID")_*_Aligned.MarkDup_Filtered.bam)"
	...
	echo "-L chr${i} \\"
	echo "-I \${BAM} \\"
	echo "-o \${ID}_chr${i}.g.vcf.gz"
	
	) > "run_GATK_HC_SA.Atelc_chr23_38_highp.sh"

	qsub run_GATK_HC_SA.Atelc_chr23_38_highp.sh
