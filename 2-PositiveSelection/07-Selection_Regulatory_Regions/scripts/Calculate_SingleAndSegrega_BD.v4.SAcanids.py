...
if (numpy.sum(BD_single) > 0) and (numpy.sum(SegregationSites) > 0):
	TotalBD=float(((numpy.sum(BD_single)/4)/sites_passing))
	Segre=float((numpy.sum(SegregationSites)/18)/sites_passing)
	SinglBySeg = TotalBD/Segre
	Missingness=float((numpy.sum(Missing)/23))
	print('%s\t%d\t%d\t%f\t%d\t%d\t%d\t%d\t%f\t%s' % (chromo,start_pos,end_pos,TotalBD,Segre,SinglBySeg,sites_present,sites_passing,Missingness,EnsemblID))


elif (numpy.sum(BD_single) == 0) and (numpy.sum(SegregationSites) > 0):
	TotalBD=0
	Segre=float((numpy.sum(SegregationSites)/18)/sites_passing)
	Missingness=float((numpy.sum(Missing)/sites_passing))
	print('%s\t%d\t%d\t%f\t%f\t%s\t%d\t%d\t%f\t%s' % (chromo,start_pos,end_pos,TotalBD,Segre,"Only_Segre",sites_present,sites_passing,Missingness,EnsemblID))

elif (numpy.sum(BD_single) > 0) and (numpy.sum(SegregationSites) == 0):
	TotalBD=float(((numpy.sum(BD_single)/4)/sites_passing))
	Segre=0
	Missingness=float((numpy.sum(Missing)/23))
	print('%s\t%d\t%d\t%f\t%f\t%s\t%d\t%d\t%f\t%s' % (chromo,start_pos,end_pos,TotalBD,Segre,"Only_SingleBD_MW",sites_present,sites_passing,Missingness,EnsemblID))

elif (numpy.sum(BD_single) == 0) and (numpy.sum(SegregationSites) == 0):
	Segre=0
	TotalBD=0\
	Missingness=float((numpy.sum(Missing)/23))
	print('%s\t%d\t%d\t%f\t%f\t%s\t%d\t%d\t%f\t%s' % (chromo,start_pos,end_pos,TotalBD,Segre,"No_variant",sites_present,sites_passing,Missingness,EnsemblID))
