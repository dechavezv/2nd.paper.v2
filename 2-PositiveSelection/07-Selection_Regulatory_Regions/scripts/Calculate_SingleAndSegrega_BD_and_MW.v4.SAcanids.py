...
if (numpy.sum(MW_single) > 0 or numpy.sum(BD_single) > 0) and (numpy.sum(SegregationSites) > 0):
	...
	Missingness=float((numpy.sum(Missing)/23))
	print('%s\t%d\t%d\t%f\t%f\t%f\t%f\t%f\t%d\t%d\t%f\t%s' % (chromo,start_pos,end_pos,TotalBD,TotalMW,Diver,Segre,SinglBySeg,sites_present,sites_passing,Missingness,EnsemblID))
	...
	Segre=float((numpy.sum(SegregationSites)/13)/sites_passing)
	Missingness=float((numpy.sum(Missing)/23))
	print('%s\t%d\t%d\t%f\t%f\t%f\t%f\t%s\t%d\t%d\t%f\t%s' % (chromo,start_pos,end_pos,TotalBD,TotalMW,Diver,Segre,"Only_Segre",sites_present,sites_passing,Missingness,EnsemblID))
...
# Add zero if Segragation is equal to zero
	...
	Segre=0
	Missingness=float((numpy.sum(Missing)/23))
	print('%s\t%d\t%d\t%f\t%f\t%f\t%f\t%s\t%d\t%d\t%f\t%s' % (chromo,start_pos,end_pos,TotalBD,TotalMW,Diver,Segre,"Only_SingleMW",sites_present,sites_passing,Missingness,EnsemblID))
	...
	print('%s\t%d\t%d\t%f\t%f\t%f\t%f\t%s\t%d\t%d\t%f\t%s' % (chromo,start_pos,end_pos,TotalBD,TotalMW,Diver,Segre,"Only_SingleBD_MW",sites_present,sites_passing,Missingness,EnsemblID))
	...
	TotalBD=0
	Missingness=float((numpy.sum(Missing)/23))
	print('%s\t%d\t%d\t%f\t%f\t%f\t%f\t%s\t%d\t%d\t%f\t%s' % (chromo,start_pos,end_pos,TotalBD,TotalMW,Diver,Segre,"No_variant",sites_present,sites_passing,Missingness,EnsemblID))
