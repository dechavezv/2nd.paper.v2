...
echo -e 'chrom\tStarWind\tEndWind\tPolym\tDiverg\tHKAratio\tTotalSites\tGoodQsites\tSitesPolym\tPercGoodQ\tENSEMBL\tGene' > HKA.BD.July2nd.txt
for i in {01..38} X;do \
cat Genes_HKA_bsve_joint_chr${i}_TrimAlt_Annot_Mask_Filter.HKA.txt | sort -k2,3 -h | uniq | perl -pe 's/\t\n/\tNA\n/g' | grep -vE 'nan' >> HKA.BD.July2nd.txt;done
