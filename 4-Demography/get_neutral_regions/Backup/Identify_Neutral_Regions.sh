...
#$ -l h_rt=23:00:00,h_data=40G,h_vmem=45G,highp
#$ -o /u/project/rwayne/snigenda/finwhale/reports/Neutral_sites.out.txt
#$ -e /u/project/rwayne/snigenda/finwhale/reports/Neutral_sites.err.txt
#$ -pe shared 4
#$ -m abe
...
if [ $REF == 'Minke' ]; then
    ...
    REPEATS_WM=${REFDIR}/GCF_000493695.1_BalAcu1.0_genomic_repeats.bed
    REPEATS_RM=${REFDIR}/GCF_000493695.1_BalAcu1.0_rm.out.bed
    CPG_REGIONS=${REFDIR}/ucsc/CpG_ExtUnmasked.bed
    CPG_REPEATS=${REFDIR}/CpG_repeats.bed
...
hqSites=${WORKDIR}/HiQualCoords/HQsitesOnly/all_HQCoords_sorted_merged.bed # bed coords (sorted, merged) of sites from previous selection of high quality coordinates (comes from the output of the script "GetHiQualCoords_20200602.sh")
...
bedtools merge -d 10 -i ${WORKDIR}/repeatRegions/all_HQCoords_min10kb_DistFromExons_noCpGIsland_noRepeat.bed > ${WORKDIR}/get_fasta/all_HQCoords_min10kb_DistFromExons_noCpGIsland_noRepeat_mergedMaxDistance10.forFasta.notForSFS.bed
...
bedtools intersect -v -a ${WORKDIR}/repeatRegions/all_HQCoords_min10kb_DistFromExons_noCpGIsland_noRepeat.bed -b ${WORKDIR}/zebra_fish/fish.matches.eval.1e-10.0based.bed > ${WORKDIR}/zebra_fish/all_HQCoords_min10kb_DistFromExons_noCpGIsland_noRepeat_noMappedFish.bed
