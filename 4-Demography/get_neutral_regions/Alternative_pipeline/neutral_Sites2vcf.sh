...
#$ -l h_rt=23:00:00,h_data=20G,h_vmem=30G,highp
#$ -o /u/project/rwayne/snigenda/finwhale/reports/Alt_GetNeutral_sites.out.txt
#$ -e /u/project/rwayne/snigenda/finwhale/reports/Alt_GetNeutral_sites.err.txt
#$ -t 81
#$ -m abe
...
VCFDIR=${WORKDIR}/neutralVCFs/ALTPIPE/noCpGRepeats
OUTDIR=${WORKDIR}/neutralVCFs/ALTPIPE
SCRIPTDIR=${HOMEDIR}/scripts/get_neutral_regions/Alternative_pipeline
...
mkdir -p ${WORKDIR}/repeatRegions/Alternative
mkdir -p ${WORKDIR}/get_fasta/Alternative
mkdir -p ${WORKDIR}/zebra_fish/Alternative
mkdir -p ${WORKDIR}/HQ_neutral_sites/Alternative # This directory will have neutral regions going through 3 checks: CpG Islands, repetitive regions, and do not blast to fish
mkdir -p ${OUTDIR}/beds

Fishy=${WORKDIR}/zebra_fish/Alternative/Alt_fish.matches.eval.1e-10.0based.bed
...
-R ${REFERENCE} \
--variant ${VCFDIR}/Alt_nocpg_repeats_SFS_${IDX}.vcf.gz  \
-XL ${Fishy} \
-L ${TenKb} \
-o ${OUTDIR}/Alt_Neutral_sites_SFS_${IDX}.vcf.gz &>> ${LOG}
...
python ${NeutralCoord_SCRIPT} --VCF ${OUTDIR}/Alt_Neutral_sites_SFS_${IDX}.vcf.gz --outfile ${OUTDIR}/beds/Alt_Neutral_sites_SFS_${IDX}.bed

bedtools merge -i ${OUTDIR}/beds/Alt_Neutral_sites_SFS_${IDX}.bed > ${OUTDIR}/beds/Alt_Neutral_sites_SFS_merge_${IDX}.bed

cat ${OUTDIR}/beds/Alt_Neutral_sites_SFS_merge_*.bed | sort -k1,1 -k2,2n > ${OUTDIR}/beds/Alt_Neutral_sites_SFS_sorted.bed

cp ${OUTDIR}/beds/Alt_Neutral_sites_SFS_sorted.bed ${WORKDIR}/HQ_neutral_sites/Alternative

cd ${WORKDIR}/HQ_neutral_sites/Alternative

cp Alt_Neutral_sites_SFS_sorted.bed Final_HQ_neutral_regions.bed

awk -F'\t' 'BEGIN{SUM=0}{ SUM+=$3-$2 }END{print SUM}' ${WORKDIR}/HQ_neutral_sites/Alternative/Final_HQ_neutral_regions.bed > ${WORKDIR}/HQ_neutral_sites/Alternative/totalPassingSequence.txt
